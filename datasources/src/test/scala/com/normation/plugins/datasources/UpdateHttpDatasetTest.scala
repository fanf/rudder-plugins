...
class UpdateHttpDatasetTest extends Specification with BoxSpecMatcher with Loggable with AfterAll {
  ...
  object NodeDataset {
    ...
    def delayResponse(resp: IO[Response[IO]]): IO[Response[IO]] = {
      timer.sleep(Random.nextInt(1000).millis.toScala).flatMap(_ => resp)
    }
  ...
  val serverR = {
    (for {
      ...
      server  = BlazeServerBuilder[IO]
                  .withExecutionContext(blockingExecutionContext)
                  .bindAny()
  ...
  withResource(serverR) { server =>
    ...
    "Getting a node" should {
      ...
      "get the node" in {
        val res =
          fetch.getNode(DataSourceId("test-get-one-node"), datasource, n1, root, alwaysEnforce, Set(), 1.second, 5.seconds)
    ...
    "The behavior on 404 " should {
      ...
      "have a working 'don't touch - not exists' option" in {
        test404prop(propName = "test-404", initValue = None, onMissing = MissingNodeBehavior.NoChange, expectMod = false) {
          props => props must havePairs(props.keySet.map(x => (x, None)).toSeq: _*)
        }
      }
